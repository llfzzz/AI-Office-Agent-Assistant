...
export function fallbackAnalysis(input, warning) {
  ...
  const meeting_understanding = {
    ...
    notes_for_extraction: warning
      ? `当前使用演示解析：${warning}`
      : '当前使用演示解析，建议配置 GPTSAPI_KEY 运行真实 Prompt 链路。',
  };
  ...
  const quality_check = {
    ...
    revision_suggestions: warning
      ? ['已降级为演示解析；配置 GPTSAPI_KEY 后可运行完整三段 Prompt 自检。']
      : [],
  };
  ...
  return {
    ...
    provider: null,
    warnings: warning ? [warning] : ['未配置 GPTSAPI_KEY，当前结果来自本地演示解析。'],
...
export function fallbackAnswer(meeting, question, warning) {
  ...
  return {
    ...
    confidence,
    source: 'demo-fallback',
    warnings: warning ? [warning] : ['未配置 GPTSAPI_KEY，当前回答来自本地会议记录匹配。'],
  };
}
...
function normalizeOfficePlan(input, selectedSkill, ragContext, warning) {
  ...
  return {
    ...
    risk_notes: warning
      ? [`当前使用演示规划：${warning}`]
      : ['演示模式只做关键词级拆解，建议配置 GPTSAPI_KEY 运行真实 Agent Plan。'],
  };
}
  ...
  return {
    ...
    quality_check: {
      ...
      revision_suggestions: agent_plan.missing_information.length
        ...
        : ['演示模式已完成基础自检；配置 GPTSAPI_KEY 后可运行完整质量检查 Prompt。'],
    },
  };
}
